<script lang="ts">
  ...
  import { getOid, isNone } from "@/lib/utils";
  ...
  async function update() {
    try {
      const productUpdate = tryUpdate();
      const res = await ApiRoutes.update_product(productUpdate);
  ...
  function updateFields(data: ProductPublic) {
    let options: OptionEntry[] = Object.keys(data.options).map((name) => {
      const values = data.options[name];
      return {
        name: name,
    ...
    let variants: VariantEntry[] = data.variants.map((variant) => ({
      sku: variant.sku,
      options: new Map(
    ...
    fields = {
      ...
      base_compare_price: data.base_compare_price,
  ...
  function tryUpdate(): ProductUpdate {
    if (!fields.name) {
      throw "Product name can't be empty";
    }
    let options = tryGetOptions();
    let variants = tryGetVariants();
    ...
    const body: ProductUpdateBody = {
      ...
      options: options === product.options ? null : options,
      variants: variants === product.variants ? null : variants,
    };

    if (isNone(body)) {
      throw "Nothing has been updated";
    }
