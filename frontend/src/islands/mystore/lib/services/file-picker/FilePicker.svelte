<script lang="ts">
    import { filePicker } from "./filePicker.store";
    ...
    async function fetchFiles(
        ...
        try {
            const response: Page<FileEntrySummary> =
                await ApiRoutes.get_all_files({
                    page: page || null,
            ...
            if (err instanceof Error) {
                ...
                throw new Error(
                    "Failed to fetch files: Unknown error occurred",
                );
            }
        }
    }
                ...
                <div class="dialog-body">
                    ...
                    {:else if files.length === 0}
                        <div class="empty">
                            ...
                            <p>
                                {#if allowedMime}
                                    No files matching "{allowedMime}" were
                                    found.
                                {:else}
                    ...
                    {:else}
                        <div class="file-grid">
                            {#each files as file (file.url)}
                                <div
                                    class="file-item"
                                    class:selected={selectedFiles.some(
                                        (f) => f.url === file.url,
                                    )}
