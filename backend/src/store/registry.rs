...
use crate::{
    utils::{error::Error, types::Result},
    AppState,
};
    ...
    pub async fn hardcoded() -> Result<Self> {
        Ok(Self(HashMap::from([
            ...
            (
                Templates::PRODUCT,
                (
                    ObjectId::from_str("681206f7e5c5c95dd5005ed5").unwrap(),
                    None,
                ),
            ),
        ])))
        ...
        if let Some((id, template)) = self.0.get_mut(&template) {
            if template.is_none() {
                ...
                let mut download_stream = bucket
                    ...
                    .await
                    .map_err(|_| Error::from("template not found".to_string()))?;
                let _ = download_stream
                    .read_to_end(&mut buf)
                    .await
                    .map_err(|_| Error::from("error reading template".to_string()))?;
